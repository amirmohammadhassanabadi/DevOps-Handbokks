...
Worker nodes are machines where **application workloads (Pods)** run.

A typical worker node contains:

* **kubelet** → manages Pods assigned to the node
* **kube-proxy** → implements Service networking *(when used)*
* **Container runtime** → runs the containers

> **The kubelet and a container runtime are required for a node to run Pods. kube-proxy may be optional depending on the cluster's networking implementation.**
